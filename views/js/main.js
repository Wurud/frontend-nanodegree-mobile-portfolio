...
var makeRandomPizza = function() {
  ...
  //Changes made: declare a new variables to be Calculated one time outside the loops.
  var ingredientItemizerOfMeats = ingredientItemizer(selectRandomMeat());
  var ingredientItemizerOfNonMeats = ingredientItemizer(selectRandomNonMeat());
  var ingredientItemizerOfCheeses = ingredientItemizer(selectRandomCheese());

  for (var i = 0; i < numberOfMeats; i++) {
    pizza = pizza + ingredientItemizerOfMeats;
  }

  for (var j = 0; j < numberOfNonMeats; j++) {
    pizza = pizza + ingredientItemizerOfNonMeats;
  }

  for (var k = 0; k < numberOfCheeses; k++) {
    pizza = pizza + ingredientItemizerOfCheeses;
  }
...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    switch (size) {
      case "1":
        //Changes made: replace all querySelector with getElementById
        //getElementById is fetching a live nodeList which is much faster than fetch a static nodeList by using querySelector
  ...
  function changePizzaSizes(size) {
    // Changes made: replace querySelectorAll with getElementsByClassName and
    // declare new variable outside the loop to hold DOM query and avoid avoid FSL.
...
//Changes made: declare pizzasDiv one time outside the loop.
...
function updatePositions() {
  ...
  //Changes made: I decompose the updatePositions loop into small parts
  //1-Replace querySelectorAll with getElementsByClassName.
  var items = document.getElementsByClassName('mover');

  //2-This will calculate array length one time.
